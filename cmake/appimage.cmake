function(make_appimage)
    ...
    file(GLOB LIBRARIES "${CMAKE_INSTALL_PREFIX}/lib/*.so*" "${CMAKE_INSTALL_PREFIX}/bin/*.so*")
    list(REMOVE_DUPLICATES LIBRARIES)

    foreach(rm_lib IN ITEMS "librt.so" "libpthread.so" "libc.so" "libstdc*" "librt-.*.so" "libpthread-.*.so" "libc-.*.so")
        message("${rm_lib}")
        list(FILTER LIBRARIES EXCLUDE REGEX ${rm_lib})
    endforeach()

    message("Installing! ${LIBRARIES}")
    file(COPY ${LIBRARIES} DESTINATION "${APPDIR}" FOLLOW_SYMLINK_CHAIN)
